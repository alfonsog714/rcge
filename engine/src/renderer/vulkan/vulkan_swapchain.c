    ...
    u32 *out_image_index)
{
    ...
    {
        RCFATAL("Failed to acquire swapchain image!");
        return FALSE;
    }
    ...
    u32 present_image_index)
{
    ...
    {
        RCFATAL("Failed to present swapchain image!");
    }

    /* This increments and loops the index. */
...
void create(vulkan_context *context, u32 width, u32 height, vulkan_swapchain *swapchain)
{
    ...
    swapchain_extent.width = RCCLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = RCCLAMP(swapchain_extent.height, min.height, max.height);
    ...
    if (!swapchain->images)
    {
        swapchain->images = (VkImage *)rcallocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }

    if (!swapchain->views)
    {
        swapchain->views = (VkImageView *)rcallocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    ...
    if (!vulkan_device_detect_depth_format(&context->device))
    {
        ...
        RCFATAL("Failed to find a supported format!");
    }
    ...
    RCINFO("Swapchain created successfully.");
}
...
void destroy(vulkan_context *context, vulkan_swapchain *swapchain)
{
    ...
    /* Only destroy the views, not the images themselves, since those are owned by the swapchain and will be
       destroyed when the swapchain is.*/
    for (u32 i = 0; i < swapchain->image_count; ++i)
    {
