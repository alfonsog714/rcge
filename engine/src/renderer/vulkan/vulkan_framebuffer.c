...
{
    out_framebuffer->attachments = rcallocate(sizeof(VkImageView) * attachment_count, MEMORY_TAG_RENDERER);
...
void vulkan_framebuffer_destroy(vulkan_context *context, vulkan_framebuffer *framebuffer)
{
    ...
    {
        rcfree(framebuffer->attachments, sizeof(VkImageView) * framebuffer->attachment_count, MEMORY_TAG_RENDERER);
