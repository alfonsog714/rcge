    ...
    vulkan_image *out_image)
{
    ...
    image_create_info.arrayLayers = 1;  // TODO: Support number of layers in image
    image_create_info.format = format;
    image_create_info.tiling = tiling;
    image_create_info.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
    image_create_info.usage = usage;
    image_create_info.samples = VK_SAMPLE_COUNT_1_BIT;         // TODO: Support configurable sample count
    image_create_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // TODO: Support configurable sharing mode
    ...
    if (memory_type == -1)
    {
        RCERROR("Required memory type not found. Image not valid.");
    }
    ...
    /* Bind the memory */
    VK_CHECK(vkBindImageMemory(context->device.logical_device, out_image->handle, out_image->memory, 0)); // TODO: Memory offset
