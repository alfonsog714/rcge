...
b8 vulkan_fence_wait(vulkan_context *context, vulkan_fence *fence, u64 timeout_ns)
{
    if (!fence->is_signaled)
    {
        ...
        case VK_SUCCESS:
            ...
            return TRUE;
        case VK_TIMEOUT:
            RCWARN("vk_fence_wait - Timed out");
            break;
        case VK_ERROR_DEVICE_LOST:
            RCERROR("vk_fence_wait - VK_ERROR_DEVICE_LOST");
            break;
        case VK_ERROR_OUT_OF_HOST_MEMORY:
            RCERROR("vk_fence_wait - VK_ERROR_OUT_OF_HOST_MEMORY");
            break;
        case VK_ERROR_OUT_OF_DEVICE_MEMORY:
            RCERROR("vk_fence_wait - VK_ERROR_OUT_OF_DEVICE_MEMORY");
            break;
        default:
            RCERROR("vk_fence_wait - An unknown error has occurred");
            break;
        }
    }
    else
